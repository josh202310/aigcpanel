...
const CloudApi = {
    async config(url: string) {
        try {
            ...
            Log.error("CloudApi.config.error", e);
            throw e;
        }
    },
    ...
    async submit(url: string, data: any) {
        try {
            ...
            Log.error("CloudApi.submit.error", e);
            throw e;
        }
    },
        ...
        try {
            ...
            if (data && data.data && data.data.logs) {
                try {
                    ...
                    Log.error("CloudApi.query.decodeError", decodeError);
                }
            }
            return data;
        } catch (e: any) {
            Log.error("CloudApi.query.error", e);
            throw e;
        }
    },
    ...
    async cancel(url: string) {
        try {
            ...
            Log.error("CloudApi.cancel.error", e);
    ...
    async upload(url: string, filePath: string) {
        try {
            ...
            Log.error("CloudApi.upload.error", e);
            throw e;
        }
    },
};

export const CloudServer = function () {
    const me = this;
    ...
    const getBaseUrl = () => {
        return this.serverConfig.cloudConfig.url;
    };
    ...
    this.init = async function () {
        // Cloud server reads config from ServerInfo.cloudConfig at runtime
    };

    this.config = async function () {
        const ret = await CloudApi.config(getBaseUrl());
        if (!ret.data) {
            throw new Error(ret.msg || "Cloud config fetch failed");
        }
    ...
    this.start = async function () {
        ...
        try {
            await CloudApi.config(getBaseUrl());
            this.send("running", this.ServerInfo);
        } catch (e) {
            Log.error("CloudServer.start.error", e);
    ...
    this.cancel = async function () {
        ...
        try {
            await CloudApi.cancel(getBaseUrl());
        } catch (e) {
            Log.error("CloudServer.cancel.error", e);
        }
    ...
    this._runCloudTask = async function (
        payload: any,
        option: {
            timeout: number;
        },
        ...
        let token = null;
        try {
            const submitRes = await CloudApi.submit(baseUrl, payload);
            if (submitRes.code !== 0) {
                throw new Error(submitRes.msg || "Cloud submit failed");
            }
        ...
        const poll = async () => {
            ...
            try {
                const queryRes = await CloudApi.query(baseUrl, token);
                if (queryRes.code !== 0) {
                    Log.error("CloudServer.query.error", queryRes);
                    pollError(queryRes.msg || "Cloud query failed");
                    return;
                } else {
                ...
                Log.error("CloudServer.poll.error", e);
                pollError(e.message || "Cloud poll failed");
            }
        };
        poll();
    };
    ...
    this._uploadIfNeeded = async function (path: string): Promise<string> {
        if (path && typeof path === "string") {
            ...
            if (await this.ServerApi.file.exists(path)) {
                const res = await CloudApi.upload(getBaseUrl(), path);
        ...
        try {
            ...
            await (async () => {
                return new Promise((resolve, reject) => {
                    ...
                    me._runCloudTask(payload, option);
                });
            })();
